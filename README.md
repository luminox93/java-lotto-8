...
#### 1-1. 구입 금액 입력
- [x] 사용자로부터 구입 금액 입력받기
- [x] 구입 금액은 1,000원 단위여야 함
- [x] 1,000원 미만 입력 불가
- [x] 양의 정수만 허용 (0, 음수 불가)
- [x] 잘못된 입력 시 에러 메시지 출력 후 재입력

#### 1-2. 당첨 번호 입력
- [x] 사용자로부터 당첨 번호 입력받기 (쉼표로 구분된 6개의 숫자)
- [x] 정확히 6개의 번호만 허용
- [x] 각 번호는 1~45 범위여야 함
- [x] 중복된 번호 입력 불가
- [x] 잘못된 입력 시 에러 메시지 출력 후 재입력
...
#### 2-1. 로또 번호 생성
- [x] 구입 금액에 따라 로또 구매 개수 계산 (금액 / 1,000원)
- [x] 구매 개수만큼 로또 자동 생성
- [x] 각 로또는 1~45 범위에서 중복 없이 6개 번호 생성
...
#### 3-2. 수익률 계산
- [x] 총 상금 계산 (각 등수별 당첨 개수 × 상금)
- [x] 수익률 계산 ((총 상금 / 구입 금액) × 100)
- [x] 소수점 첫째 자리까지 표시 (둘째 자리에서 반올림)
...
#### 5-2. 재입력 처리
- [x] 구입 금액 오류 시 구입 금액만 재입력
- [x] 당첨 번호 오류 시 당첨 번호만 재입력
- [x] 보너스 번호 오류 시 보너스 번호만 재입력
- [x] 예외 발생 후 프로그램 종료하지 않고 계속 진행
